...
void IdeHost::HandleForceUpdate() {
    std::thread([this]() {
        ...
        if (ofs.is_open()) {
            ...
            ofs << "taskkill /F /IM msedgewebview2.exe /T >nul 2>&1\n";
            ...
            ofs << "taskkill /F /IM CodeFork.exe /T >nul 2>&1\n";
            ofs << "taskkill /F /IM msedgewebview2.exe /T >nul 2>&1\n";
            ofs << "del /f /q /s \"%INSTALL_DIR%\\*\" >nul 2>&1\n";
            ofs << "for /d %%p in (\"%INSTALL_DIR%\\*\") do rd /s /q \"%%p\" >nul 2>&1\n";
            ofs << "rd /s /q \"%INSTALL_DIR%\" >nul 2>&1\n";
            ofs << "if exist \"%INSTALL_DIR%\" (\n";
            ofs << "    set /a \"WIPE_TRY+=1\"\n";
            ofs << "    if %WIPE_TRY% leq 5 (\n";
            ofs << "        timeout /t 1 /nobreak >nul\n";
            ofs << "        goto wipe_retry\n";
            ofs << "    )\n";
            ofs << ")\n";
